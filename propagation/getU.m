...
ti = opt.tempsfixe_valeur*optimParam(1,3)/(sum(optimParam(1:end-1,3))+opt.Nlignes*optimParam(end,3));
U(:,:,1) =  getRelaxMat(spins,ti,EXPMAT) * getExcMat(opt,spins,opt.mu*optimParam(1,1:2));



for p = 2:opt.Np
    ti = opt.tempsfixe_valeur*optimParam(p,3)/(sum(optimParam(1:end-1,3))+opt.Nlignes*optimParam(end,3));
